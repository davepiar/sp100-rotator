...
class FMPClient:
    ...
    def get_earnings_calendar(self, from_date: str, to_date: str) -> Optional[list[dict]]:
        ...
        Returns:
            List of earnings event dicts (v3-compatible field names) or None.
            Each dict contains: date, symbol, eps, epsEstimated, revenue,
            revenueEstimated. The /stable/* endpoint dropped the bmo/amc
            `time` field; callers' inline checks default to "" -> "unknown".
        """
        ...
        for row in data:
            adapted.append({
                ...
                # `time` (bmo/amc) absent in /stable/earnings-calendar.
            })
    ...
    def get_company_profiles(self, symbols: list[str]) -> dict[str, dict]:
        ...
        Returns:
            Dict mapping symbol -> profile dict with v3-compatible aliases:
